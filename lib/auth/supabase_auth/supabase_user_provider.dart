...
class ProjectoFSupabaseUser extends BaseAuthUser {
  ProjectoFSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> projectoFSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProjectoFSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
